...
#include <string.h>
using namespace std;
/* Busca binaria de registros em Arquivo Indexado */
...
void busca_aleat (struct indice idx[], struct clientes cli[], int cont, int cod){
    int i = 0, f = cont - 1;
    int m = (i + f) / 2;
    for (; f >= i && cod > idx[i].codigo; m = (i + f) / 2){
    	if (cod > idx[i].codigo)
    		i = m + 1;
    	else 
    		f = m - 1;
...
int main(){
    ...
    for (int codpesq = 9; codpesq != 0;){
        ...
        cin >> codpesq;
        if (codpesq != 0)
            busca_aleat (ind, dados, contador, codpesq);
    }
}